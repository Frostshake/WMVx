...
namespace core {
	...
	public:
		...
		void load(const GameFileSystem* const fs) override {
			...
			animationDataDB = std::make_unique<ModernWDBDefsAnimationDataDataset<ModernWDBDefsAnimationDataRecordAdaptor>>(cascFS, version, "Support Files\\bfa\\animation-names.csv"); //TODO find DF animation names
