...
#pragma pack(push, 1)
	...
	struct AnimationDataRecord : public FixedRecord<AnimationDataRecord> {
		...
		static_assert(DBCFormat::recordSizeDest(schema, DBCVersion::CATA_PLUS) == sizeof(data));
	};
	...
	struct ChrRacesRecord : public FixedRecord<ChrRacesRecord> {
		...
		struct Data {
			...
			uint32_t alliance;
			DBCLangString<DBCVersion::CATA_PLUS> nameLang;
			DBCLangString<DBCVersion::CATA_PLUS> nameFemaleLang;
			DBCLangString<DBCVersion::CATA_PLUS> nameMaleLang;
			string_data_t facialHairCustomization[2];
			string_data_t hairCustomization;
			uint32_t raceRelated;
			uint32_t unalteredVisualRaceId;
			uint32_t uaMaleCreatureSoundDataId;
			uint32_t uaFemaleCreatureSoundDataId;
		} data;
		...
		constexpr static Schema schema = Schema(
			...
			Field::value<decltype(data.uaMaleCreatureSoundDataId)>(),
			Field::value<decltype(data.uaFemaleCreatureSoundDataId)>()
		);
	...
	struct CreatureModelDataRecord : public FixedRecord<CreatureModelDataRecord> {
		...
		constexpr static Schema schema = Schema(
			...
			Field::value<decltype(data.geoBoxMin)>(),
			Field::value<decltype(data.geoBoxMax)>(),
